    ...
    private func embed() {
        ...
        if (waypoints.count > 0) {
            ...
            for item in waypoints {
                let itemDict = item as? [String:Double] ?? [:]
                let point = Waypoint(coordinate: CLLocationCoordinate2D(latitude: itemDict["latitude"]!, longitude: itemDict["longitude"]!))
                point.coordinateAccuracy = 1
                wayTypesPointArray.append(point)
            }
            options = NavigationRouteOptions(waypoints: wayTypesPointArray)
        }
        else {
            ...
            options = NavigationRouteOptions(waypoints: [originWaypoint, destinationWaypoint])
        }
